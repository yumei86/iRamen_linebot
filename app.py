...
#----------------選擇湯頭/直接推薦介面-----------------
    ...
    #北部
        ...
        "新北市": ["豚骨", "醬油", "雞白湯", "雞骨魚介", "沾麵", "雞清", "淡麗系", "煮干", "素食", "其他"],
        "基隆市" : ["豚骨", "素食", "限定"],
        "桃園市" : ["豚骨", "醬油", "雞白", "魚介", "沾麵", "雞清", "柑橘", "淡麗", "煮干", "其他"],
        "苗栗縣" : ["豚骨", "咖哩","雞骨"],
        "新竹縣" : ["豚骨"],
        "新竹市" : ["豚骨", "醬油", "二郎", "沾麵" , "雞清", "家系" , "素食", "背脂", "淡麗系" , "其他"],

        #中部
        "台中市" : ["豚骨", "醬油", "雞白", "魚介", "沾麵", "雞清", "家系", "辣味噌", "泡系", "其他"],
        "彰化縣" : ["豚骨", "雞白湯", "家系", "鴨清", "鴨白", "限定", "雞湯"],
        "南投縣" : ["豚骨", "其他"],
        "雲林縣" : ["雞白", "魚介雞湯", "限定"],

        #南部
        "嘉義市" : ["豚骨", "沾麵", "家系", "二郎風", "限定", "變化"],
        "台南市" : ["豚骨", "雞白湯", "豚骨魚介", "沾麵", "二郎", "素食", "咖哩", "淡麗系", "煮干", "其他"],
        "高雄市" : ["豚骨", "醬油", "雞白湯", "豚骨魚介", "沾麵", "雞清", "家系", "雞豚", "淡麗", "其他"],
        "屏東縣" : ["豚骨", "醬油", "雞白", "雞清", "家系", "雞濃", "限定", "其他"],
...
#----------------輸入關鍵字找尋店家-----------------
    ...
    if ('湯頭推薦'not in event.message.text and ':' in event.message.text) and ':' not in event.message.text[0] and ':' not in event.message.text[-1]:
        ...
        if ((select_first_param == '直接推薦') or (select_first_param == '看更多推薦')) and select_second_param in city_name:                
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text = f"1"))
        elif select_first_param in city_name and select_second_param in city_soup[select_first_param]:
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text = f"2"))
        else:
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text = f"3"))
